...
def car_in_view(x, y, c, s, other_cars, max_distance=60.0, car_length=5.0, car_width=2.0, gx=0, gy=0):
    # corners = get_rotated_points(x, y, c, s, get_corners(x, y))
    others = []
    for car in other_cars:
        if car[0] == car[1] == car[2] == car[3] == 0: continue
        if car[0] == gx and car[1] == gy: continue
...
class Agent:
    ...
    def process_obs(self, obs):
        ...
        check = car_in_view(player[0], player[1], player[4], player[5], near_xycs, max_distance=60.0, gx=goal[0], gy=goal[1]) # min(6.0, abs(speed) * 2)
    ...
    def get_reward(self, obs, action, obs_):
        ...
        if self.dis >= 50:
            ...
            if obs_["is_crashed"]: return -0.1
            dis = ((player[0] - goal_spot[0])**2 + (player[1] - goal_spot[1])**2)**0.5
            dis_pre = ((player_pre[0] - goal_spot[0])**2 + (player_pre[1] - goal_spot[1])**2)**0.5

            # print(((dis_pre - dis) / 5) * (1 / (dis + 0.1)))


            if dis < dis_pre: return ((dis_pre - dis) / 5) * (1 / (dis + 0.1))
            else: return -max(0.1, ((dis - dis_pre) / 5) * (1 / (dis + 0.1)))
        

    def act(self, obs):
        obs_ = normalize_obs(self.process_obs(obs))
        if self.dis < 50:
            return self.DQN2.choose_action(obs_)
    ...
    def save_model2(self, episode):
        model_filename = f"model2_{episode}.pth"
        torch.save(self.DQN2.policy_net.state_dict(), model_filename)
        print(f"Model saved as {model_filename}")
    ...
    def continue_train(self, model):
        self.DQN.epsilon = 0.1
        self.DQN2.epsilon = 0.9
        self.DQN2.epsilon_decay = 0.9
        self.DQN.policy_net.load_state_dict(torch.load(model))
        self.DQN2.policy_net.load_state_dict(torch.load(model))
        ...
        try:
            while True:
                ...
                while True:
                    ...
                    self.dis = ((obs_[0] ** 2) + (obs_[1] ** 2)) ** 0.5
                    ...
                    if self.dis >= 50:
                        ...
                        self.DQN2.store_transition(obs, torch.tensor([[action]], device=device, dtype=torch.long), reward, obs_)

                    if len(self.DQN2.memory) >= 1000 and cur_step % UPDATE_INTERVAL == 0:
                        self.DQN2.learn()
                        print("loss : ", self.DQN2.loss.item())
                        print("epsilon : ", self.DQN2.epsilon)
                        if cur_step % (UPDATE_INTERVAL * 10) == 0: 
                            pass
                            #self.save_model2(0)
...
if __name__ == "__main__":
    agent = Agent()
    agent.continue_train("model_0.pth")
    agent.train()
    evaluate(agent)
